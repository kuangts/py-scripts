...
def choleskySolve(M, b):
    ...
    # a = np.empty((M.shape[1], b.shape[1]))
    # a[:] = np.nan
...
def nonrigidIcp(sourcemesh,targetmesh):
    ...
    sourcemesh = o3d.geometry.TriangleMesh(
        vertices=o3d.cpu.pybind.utility.Vector3dVector(sourcemesh.vertices),
        triangles=o3d.cpu.pybind.utility.Vector3iVector(sourcemesh.triangles),
        )
    targetmesh = o3d.geometry.TriangleMesh(
        vertices=o3d.cpu.pybind.utility.Vector3dVector(targetmesh.vertices),
        triangles=o3d.cpu.pybind.utility.Vector3iVector(targetmesh.triangles),
        )
    ...
    for num_,alpha_stiffness in enumerate(alphas):
        ...
        for i in range(3):
            ...
            X = choleskySolve(A.toarray(), B.toarray())
