...
class Dictionary:
    ...
    def __getitem__(self, item):
        ...
        print(f'{item} is not in dicom dictionary')
        return 'UNKNOWN'
...
def read(dicom_series_file_or_dir, return_image=True, return_info=False, **dicomargs):
    ...
    if not series_IDs:
        ...
        return None
    if len(series_IDs) > 1:
        print('WARNING: more than one series found, using the longest')
        id_len = [len(sitk.ImageSeriesReader.GetGDCMSeriesFileNames(dicom_series_file_or_dir, id)) for id in series_IDs]
        series_IDs = [series_IDs[np.array(id_len).argmax()]]
