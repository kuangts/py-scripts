...
from pointset import namedarray
...
class Library(frozenset): # immutable set containing immutable landmark entries
    ...
    def update_coordinates(self, lmks:landmark):
        lmk_set = self.filter(lambda x:x.Name in lmks['name'])
        lmk_set = {x._replace(Coordinate=tuple(x['coordinates'].tolist())) for x in lmk_set}
        lmk_set.__repr__ = lambda x:x.string(sort_key=attrgetter('ID'), group_key=None, group=False, id=False, definition=False)
        return lmk_set


    def to_array(self):
        return landmark([*{x.Name:x.Coordinate for x in self.sorted()}.items()])

    def __new__(cls, db_location_or_existing_set=default_db_location):
        if isinstance(db_location_or_existing_set, Library):
        ...
        if isinstance(db_location_or_existing_set, str): 
            ...
            assert all(isinstance(x, cls.LDMK) for x in items), f'cannot create library with input {db_location_or_existing_set}'
